...
app = FastAPI(
    ...
    description=(
        ...
        "Provides async CRUD operations for managing tasks, input validation"
        "via Pydantic and interactive API documentation (Swagger UI & ReDoc). "
    ),
...
@app.post("/set-cookie")
def cookie_set_response(response: Response):
    response.set_cookie(key="token", value=str(uuid.uuid4()))
    return {"message": "Cookie set"}


@app.post("/get-cookie")
def cookie_get_response(request: Request):
